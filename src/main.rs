...
fn write_global_variable(node: tree_sitter::Node, buf: &mut String, source: &String){
    let var_type = node.child_by_field_name("type").unwrap().utf8_text(source.as_ref()).unwrap();
    ...
    for child in node.children(&mut cursor)
    {
        ...
        let var_name = child.child_by_field_name("name").unwrap().utf8_text(source.as_ref()).unwrap();
